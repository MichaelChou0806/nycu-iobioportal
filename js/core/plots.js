...
function divColor(v, vmax) {
  ...
  if (t >= 0) { const r = 255 + (239 - 255) * t, g = 255 + (68 - 255) * t, b = 255 + (68 - 255) * t; return `rgb(${r | 0},${g | 0},${b | 0})`; }
  const u = -t; const r = 255 + (59 - 255) * u, g = 255 + (130 - 255) * u, b = 255 + (246 - 255) * u; return `rgb(${r | 0},${g | 0},${b | 0})`;
}
...
export function heatmapSVG(rows, cols, cells, opts) {
  ...
  s += `<defs><pattern id="hatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
        ...
        <stop offset="0%" stop-color="${divColor(vmax, vmax)}"/><stop offset="50%" stop-color="#ffffff"/><stop offset="100%" stop-color="${divColor(-vmax, vmax)}"/></linearGradient></defs>`;
  ...
  for (let r = 0; r < rows.length; r++) {
    for (let c = 0; c < cols.length; c++) {
      ...
      if (cell.state === "nodata") {
        s += `<rect x="${x}" y="${y}" width="${cw}" height="${ch}" fill="url(#hatch)" stroke="#fff"><title>${cell.tip || "no data"}</title></rect>`;
      } else {
        const op = cell.state === "weak" ? 0.45 : 1;
        s += `<rect x="${x}" y="${y}" width="${cw}" height="${ch}" fill="${divColor(cell.value, vmax)}" fill-opacity="${op}" stroke="#fff"><title>${cell.tip || ""}</title></rect>`;
  ...
  s += `<rect x="${lx}" y="${ly}" width="12" height="${lh}" fill="url(#cbar)" stroke="#c9ced6"/>`;
...
export function multiBarSVG(groups, opts) {
  ...
  let yMax = 0;
  groups.forEach(g => { if (g.state !== "nodata") yMax = Math.max(yMax, g.m + (g.sd || 0)); });
  ...
  groups.forEach((g, i) => {
    ...
    s += `<rect x="${cx - bw / 2}" y="${top}" width="${bw}" height="${base - top}" fill="#64748b" fill-opacity="${op}"/>`;
    const eT = yS(g.m + (g.sd || 0)), eB = yS(Math.max(0, g.m - (g.sd || 0)));
